  ...
  <div class="win-remote" style="width: 100%" ref="remoteWrapper">
    <div class="win-remote__patient">
      <div
        ...
        :class="{
          'input-wrap-stretch': componentShowMethod === 'tag',
          'fix-input-width': fixInputWidth
        }"
      >
        <el-popover
      ...
      <div
        class="win-remote__patient--list"
        :class="{ 'only-more': componentShowMethod === 'onlyMore' && !fixInputWidth }"
      >
        <el-button
          ...
          v-win-hotkey:[{eventName:`${btn.indexSearchMethodId}`,eventDesc:`${btn.indexSearchMethodDispName||btn.indexSearchMethodName}`,defaultHotKey:handleHotKey(btn),disRegister:!handleHotKey(btn),args:btn}].component="
            readCardByTag
          "
...
<script>
  ...
  import iconRemote from '../assets/images/remote.svg';
  import iconRemoteActive from '../assets/images/remote-active.svg';
  import iconEnter from '../assets/images/enter.svg';
  import iconEnterActive from '../assets/images/enter-active.svg';
  import iconClick from '../assets/images/click.svg';
  import iconClickActive from '../assets/images/click-active.svg';
  ...
  export default {
    ...
    methods: {
      ...
      getIdentityList() {
        getSettingCardList({
          ...
          if (res.success) {
            ...
            // 默认第一个选中
            this.cardType =
              this.cardTypeList.filter((cardType) => {
      ...
      handleEnter() {
        ...
        if (
          interfaceOriginFlag === this.primaryKey &&
          searchMethodType.enter === indexSearchMethodTypeCode
        ) {
          this.requestHybridInterface();
        } else {
          // 身份标识检索 && 院内发行就诊卡
          const isReadCard =
            this.cardType.identitySearchFlag === this.primaryKey &&
            this.cardType.identitySearchSettings[0].identitySearchTypeCode === '152690';
          this.getRemoteInfo(
            this.inputKeyword,
            this.computedIdentitySearchTypeCode(this.cardType),
            null,
            isReadCard
          );
        }
      ...
      tagComputed(cardTypeList = []) {
        ...
        let allBtnWidth = 0;
        let btnTiledNum = 0;
        const { btnFontSize, paddingSpace, moreWidth, iconWidth, marginLeft, borderWidth } =
          this.tagComputedOption;
        for (var i = 0; i < cardTypeList.length; i++) {
          const name =
            cardTypeList[i].indexSearchMethodDispName ||
            cardTypeList[i].indexSearchMethodName ||
            '';
          allBtnWidth +=
            name.length * btnFontSize + iconWidth + paddingSpace + marginLeft + borderWidth;
