<template>
    <el-popover
            v-model="showQueryPopover"
            v-bind="attrs">
        <!--        内容-->
        <div class="home-query-setting">
            <div class="scroll-content">

              <div class="query-item" style="margin-bottom:20px" v-if="plan && (mode === 'detail' || mode === 'edit')">
                <div class="query-item-left">筛选方案</div>
                <div class="query-item-right">
                    <el-input size="small" clearable style="display:inline-block;width:200px;margin-top: -5px;" v-model="plan.queryName"/>
                </div>
              </div>
                ...
                <div v-show="inpatStatusSubItem.list" class="query-item sub">
                    ...
                    <span
                      v-show="[2,3,4].includes(inpatientStatus.activeIndex)"
                      class="details-date">
                      指定起止日期
                      <el-date-picker
                        style="width:400px"
                ...
                <div v-show="inpatientStatus.activeIndex===2" class="query-item sub out-reason">
                    ...
                    <div class="query-item-right">
                    <span :class="{'query-item-cell': true, 'is-active': i===outAreaReason.activeIndex}"
                          @click="handleQueryItem(outAreaReason, cell, '', i)"
                          v-for="(cell, i) in outAreaReason.list"
                          :key="i">{{cell.label}}</span>
                    </div>
                </div>
                <div v-show="inpatientStatus.activeIndex===0" :class="{disable:isPatientList}"  class="query-item"
                     v-for="(item, index) in queryData" :key="index">
                    <div class="query-item-left">{{item.name}}</div>
                    <div class="query-item-right">
                                <span :class="{'query-item-cell': true, 'is-active': i===item.activeIndex}"
                                      @click="handleQueryItem(item, cell, index, i)" v-for="(cell, i) in item.list"
                                      :key="i">{{cell.label}}</span>
                    </div>
                </div>
                <div v-show="inpatientStatus.activeIndex===0" v-for="item in queryList"
                     :key="item.inpatQueryConditionClassId" class="query-item">
                    <div class="query-item-left">{{item.queryConditionClassName}}</div>
                    <div class="query-item-right">
                    <span class="query-item-cell"
                          :class="{'query-item-cell': true, 'is-active': (item.activeIndex===''||item.activeIndex===undefined)}"
                          @click="handleQueryItem(item, cell, '', '')">全部</span>
                        <span :class="{'query-item-cell': true, 'is-active': i===item.activeIndex}"
                              @click="handleQueryItem(item, cell, '', i)"
                              v-for="(cell, i) in item.inpatQueryConditionTags"
                              :key="i">{{cell.tagName}}</span>
                    </div>
                </div>
              <!-- 其他状态 -->
            ...
            <div class="footer-content" v-if="mode === 'detail' || mode === 'edit'">
              <div></div>
                <div>
                    ...
                    <el-button class="btn" type="primary" :disabled="!plan || !plan.queryName" @click="edit">修改</el-button>
            ...
            <div class="footer-content" v-else>
                ...
                <div>
                    <el-button class="btn" type="default" @click="closeSelf">取消</el-button>
                    <el-button class="btn" v-if="showQueryBtn" type="primary" @click="handleQuery">查询</el-button>
                </div>
            </div>
            <query-dialog
                    :is-nurse="isNurse"
                    ...
                    @updateList="()=>{this.$emit('updateList');this.closeSelf()}"
                    :saveCondition="saveCondition"
                    v-bind="$attrs"/>
        </div>
        <!--        内容end-->
        <div :class="{'search-confition-btn': true, 'is-active_sort': showQueryPopover}" slot="reference">
...
import { reqQueryCondition, requestUpdatewQuery, requestDelPlan } from '../../../src/api/req'
const maps = {
  in: '399544423',
  waitin: '399544424',
...
export default {
  ...
  methods: {
    ...
    async remove(){
      let val = this.plan
			...
			this.$confirm(`确定删除排序计划 [${val.queryName}]吗？`, '提示', {
				...
				let data = await requestDelPlan({ inpatQuerySchemeId: val.inpatQuerySchemeId, type: this.type })
     ...
     switch(condition.type) {
       ...
       case 'in':
        ...
        if(condition.dateType === '1') {
          ...
          this.$nextTick(()=>{
            this.inpatStatusSubItem.activeIndex = this.inpatStatusSubItem.list.findIndex(item => item.value === condition.dateRange)
          })
        }
        ...
        if(tags) {
          tags.forEach(tag => {
            ql.forEach((item, qlIndex) => {
              const idx = item.inpatQueryConditionTags.findIndex(tagItem => tagItem.tagId === tag.tagId && tagItem.inpatQueryConditionTagId === tag.inpatQueryConditionTagId)
              if(idx !== -1) {
       ...
       case 'changearea':
       case 'discharged':
          this.inpatientStatus.activeIndex = condition.type === 'outarea' ? 2 : condition.type === 'changearea' ? 3 : 4
          let index = this.outAreaReason.list.findIndex(item => item.value === condition.dischargeOutcomeCode)
          this.outAreaReason.activeIndex = 0
          if(index !== -1) {
            this.$nextTick(()=>{
          ...
          if(condition.timeRange) {
            ...
            let index = this.inpatStatusSubItem.list.findIndex(item => item.value === condition.dateRange)
            this.inpatStatusSubItem.activeIndex = 0
            if(index !== -1) {
              this.$nextTick(()=>{
    ...
    async handleQuery() {
      ...
      delete data.tagIds;
      this.$emit('selfQuery', { queryName: '其他', queryCondition: JSON.stringify(data), queryIds });
