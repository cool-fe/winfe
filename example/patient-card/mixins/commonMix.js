...
export default {
  ...
  methods: {
      ...
      if (res.success) {
        ...
        options = options.map(option => {
          return option.map(item => {
            const tableColumn = tableColumnList.find(column => { return column.columnIndex === item.columnIndex })
            if (tableColumn) {
              return {
        ...
        if (this.options.length) {
          // 档案是否已停用
          const filter = this.tableColumnList.filter((item) => {
            return item.columnChineseName === '启用标识'
          })
    ...
    computedIdentitySearchTypeCode (cardType) {
      ...
      const searchSettings = cardType.identitySearchFlag === primaryKey ? identitySearchSettings : nonIdentitySearchSettings
      if (!searchSettings || searchSettings.length === 0) return ''
      const identitySearchTypeList = searchSettings.filter(item => item.defaultFlag === primaryKey)
      // 没有默认值但是数组有值的情况取第一个
      const attr = cardType.identitySearchFlag === primaryKey ? 'identitySearchTypeCode' : 'nonIdSearchTypeCode'
